...
# Get apps
RUN bench get-app hrms --skip-assets --branch=version-15 && rm -rf apps/hrms/.git && \
    bench get-app lending --branch=version-15 && rm -rf apps/lending/.git && \
    bench get-app telephony && rm -rf apps/telephony/.git && \
    bench get-app helpdesk && rm -rf apps/helpdesk/.git && \
    ...
    mv apps/one_fm/hrms sites/assets/
